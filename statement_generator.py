...
def decide_winner(user_choice,computer_choice):
    if user_choice == computer_choice:
        ...
        computer_details['games_drawn'] += 1;
        print('draw');
    elif (user_choice == "rock" and computer_choice == "scissors") or (user_choice == "paper" and computer_choice == "rock") or (user_choice == "scissors" and computer_choice == "paper"):
        user_details['games_won'] += 1;
        computer_details['games_lost'] += 1;
        print('user winner');
    else:
        computer_details['games_won'] += 1;
        user_details['games_lost'] += 1;
        print('computer win')

    user_details['number_of_rounds'] +=1;
    computer_details['number_of_rounds'] += 1;

def display_results(user):
    print('User: ',user['name'], ' Games won: ' , user['games_won'],
          ' Games lost: ',user["games_lost"], 'Games drawn' ,user["games_drawn"]);
...
def start_game():
    ...
    if amount_of_chosen_rounds != "": #user selected specific number of rounds
        while rounds <= amount_of_chosen_rounds:
            ...
            display_results(computer_details);
            rounds += 1
    else:
        while True:
